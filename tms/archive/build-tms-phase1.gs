...
function repairForm6Link() {
  ...
  const TARGET_NAME = 'FORM 6 RESPONSES — WEATHER LOG';
  const existingTarget = ss.getSheetByName(TARGET_NAME);

  if (existingTarget) {
    // Target sheet already exists — the orphaned Form Responses N sheet is a duplicate.
    // Delete the orphan and leave the existing named sheet in place.
    ss.deleteSheet(responseSheet);
    Logger.log('Deleted orphaned response sheet: ' + responseSheet.getName());
    Logger.log('Existing sheet "' + TARGET_NAME + '" retained as response destination.');
  } else {
    // No named sheet yet — rename the Form Responses sheet as originally intended.
    responseSheet.setName(TARGET_NAME);
    responseSheet.setTabColor('#2a5a48');
    Logger.log('Renamed response sheet to: ' + TARGET_NAME);
  }
